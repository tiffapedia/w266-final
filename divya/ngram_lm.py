...
class BaseLM(object):
    ...
    def get_seq_probability(self, seq, n_order=3, verbose=False):
        """Compute log probability (base 2) of the given sequence.

        Args:
            seq: sequence of words (tokens) to calculate probabilities
            verbose: if true, will print per-token probabilities

        Returns:
            (probabilities, num_real_tokens)
            probabilities: prbabilities list for the sequencence of words
            num_real_tokens: number of tokens scored, excluding <s> and </s>
        """
        context_size = n_order - 1
        count = 0
        probabilities = []
        # Start at third word, since we need a full context.
        for i in range(context_size, len(seq)):
            ...
            context = seq[i-context_size:i]
            if n_order == 3:
                proba = self.next_word_proba(seq[i], context)
            if n_order == 2:
                proba = self.next_word_proba_bigram(seq[i], context)
            probabilities.append(proba)
            count += 1
            # DEBUG.
            if verbose:
                print("P({:s} | {:s}) = {:.03f}".format(seq[i], " ".join(context), proba))
...
class AddKTrigramLM(BaseLM):
    ...
    def __init__(self, tokens):
        ...
        for context,w_counts in self.counts.items():
            self.context_totals[context] = sum(w_counts.values())
    ...
    def next_word_proba(self, word, seq):
        ...
        c_w1w2 = self.counts.get(context)
        c_w1w2w = 0.0
        if c_w1w2 is not None:
            c_w1w2w = c_w1w2.get(word,0.0)
        
        c_w1w2total = self.context_totals.get(context,0.0)
        #if c_w1w2total is None:
            #c_w1w2total = 0.0
        
        proba = (c_w1w2w + k)/(c_w1w2total + k*self.V)
        
        #print('c_w1w2w:',c_w1w2w)
...
class KNTrigramLM(BaseLM):
    ...
    def __init__(self, tokens):
        ...
        for context,w_counts in self.counts.items():
            ...
            self.context_nnz[context] = len(w_counts)
         
        # Compute type fertilities, and the sum z_tf.
        for word, preceding in self.type_contexts.items():
            self.type_fertility[word] = len(preceding)
    ...
    def kn_interp(self, word, context, delta, pw):
        ...
        # get context totals for the context passed
        context_totals = self.context_totals.get(context)
        # if contaxt total is zero, return the probability
        if context_totals is None:
            return pw
        
        # get counts for the given context and word
        context_nnz = self.context_nnz.get(context,0)
        context_counts = self.counts.get(context)
        context_word_count = 0
        if context_counts is not None:
            context_word_count = context_counts.get(word,0)
        
        # KNTrigram model
        alpha = (context_nnz * delta)/context_totals
        discount_proba = max(0,context_word_count - delta)/context_totals
        p_kn = discount_proba + (alpha * pw)
    ...
    def next_word_proba_bigram(self, word, seq):
        """Compute next word probability with KN backoff smoothing.

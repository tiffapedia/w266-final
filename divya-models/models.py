...
def embedding_layer(ids_, V, embed_dim, init_scale=0.001):
    ...
    W_embed_ = tf.get_variable("W_embed", initializer=tf.random_uniform_initializer(-init_scale, init_scale), 
                               shape=[V, embed_dim])
...
def fully_connected_layers(h0_, hidden_dims, activation=tf.tanh,
    ...
    for i, hdim in enumerate(hidden_dims):
        ...
        if dropout_rate > 0:
            h_ = tf.layers.dropout(h_, rate=dropout_rate, training=is_training, name="dropout")
...
def softmax_output_layer(h_, labels_, num_classes):
    ...
    with tf.variable_scope("Logits"):
        ...
        batch_size, d_dim = h_.get_shape().as_list()
        W_out_ = tf.get_variable("W_out", initializer=tf.random_normal_initializer(), 
                                 shape = [d_dim, num_classes])
        b_out_ = tf.get_variable("b_out", initializer=tf.zeros_initializer(), shape = [num_classes])
...
def BOW_encoder(ids_, ns_, V, embed_dim, hidden_dims, dropout_rate=0,
    ...
    with tf.variable_scope("Embedding_Layer"):
        #### YOUR CODE HERE ####
        xs_ = embedding_layer(ids_, V, embed_dim) #None  # replace with a call to embedding_layer
        #### END(YOUR CODE) ####
    ...
    zero_out = tf.multiply(xs_, mask_)

    # Sum embeddings: [batch_size, max_len, embed_dim] -> [batch_size, embed_dim]
    sum_embed = tf.reduce_sum(zero_out, 1)

    # Build a stack of fully-connected layers
    h_ = fully_connected_layers(sum_embed, hidden_dims, dropout_rate=dropout_rate, is_training=is_training)

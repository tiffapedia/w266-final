...
def pad_np_array_float(example_ids, max_len=250, pad_id=0.0):
    """Pad a list of lists of ids into a rectangular NumPy array.
    ...
    Returns: (x, ns)
        ...
        ns: [num_examples] NumPy array of sequence lengths (<= max_len)
    """
    arr = np.full([len(example_ids), max_len], pad_id, dtype=np.float32)
    ns = np.zeros([len(example_ids)], dtype=np.float32)
    for i, ids in enumerate(example_ids):
        cpy_len = min(len(ids), max_len)
        arr[i,:cpy_len] = ids[:cpy_len]
        ns[i] = cpy_len
...
def rnnlm_batch_generator(ids, batch_size, max_time):
    ...
    #print("target_y:",sess.run(target_y))
